...
class Product:
    ...
    def tax_for(self, amount):
        if self.category == FOOD_CATEGORY:
            return amount * FOOD_TAX_RATE
        return amount * TAX_RATE

class OrderItem:
    """Represents an item in an order, which has a product and a quantity."""
...
class Customer:
    """Base customer tier. Subclasses provide tier-specific rules."""
...
class Order:
    """An order has a customer and many order items."""
    ...
    def __init__(self, customer, items):
        ...
        if not all(isinstance(item, OrderItem) for item in items):
            raise TypeError("All order items must be OrderItem objects.")
    ...
    def discount(self):
        sub = self.subtotal()
        discount = sub * self.customer.discount_rate(sub)
    ...
    def tax(self):
        return sum(
            item.product.tax_for(item.line_total())
...
def make_customer(name, tier):
    """Create the appropriate Customer subclass for a tier."""
    customer_classes = {
        "none": Customer,
        "silver": Silver,
        "gold": Gold,
    ...
    if tier not in customer_classes:
        raise ValueError("Unknown customer tier.")
    return customer_classes[tier](name)


def make_orders():
    """Convert the legacy data into the refactored object model."""
    products = [
